...
class _SignupPageState extends State<SignupPage>
  ...
  void _signUp() async {
    ...
    if (emailQuery.docs.isNotEmpty || usernameQuery.docs.isNotEmpty) {
      ...
      _showErrorPopup('Email or Username is already in use.');
      return; // Exit the function early
    }

    // Proceed with signup if no error
    User? user =
    ...
    if (user != null) {
      ...
      _showErrorPopup('Failed to create an account. Please try again.');
    }
  }

  Future displayUsername(String username, String location) async {
    await FirebaseFirestore.instance.collection('myUsers').add({
      'username': username,
      'lastLocation': location,
    });
